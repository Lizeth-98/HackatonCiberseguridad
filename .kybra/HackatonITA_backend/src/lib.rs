...
struct StableBTreeMap0ValueType((String));
impl CdkActTryIntoVmValue<&rustpython::vm::VirtualMachine, rustpython::vm::PyObjectRef>
...
async fn _cdk_user_defined_get(_cdk_user_defined_key: String) -> (Option<String>) {
    let interpreter = unsafe { INTERPRETER_OPTION.as_mut() }
        .unwrap_or_trap("SystemError: missing python interpreter");
    let vm = &interpreter.vm;
    let params = (_cdk_user_defined_key.try_into_vm_value(vm).unwrap_or_trap(),);
    call_global_python_function("get", params)
        .await
        .unwrap_or_trap()
}
#[ic_cdk_macros::query(name = "getAllZonas")]
#[candid::candid_method(query, rename = "getAllZonas")]
async fn _cdk_user_defined_getAllZonas() -> (Vec<String>) {
    let interpreter = unsafe { INTERPRETER_OPTION.as_mut() }
        .unwrap_or_trap("SystemError: missing python interpreter");
    let vm = &interpreter.vm;
    let params = ();
    call_global_python_function("getAllZonas", params)
        .await
        .unwrap_or_trap()
}
#[ic_cdk_macros::query(name = "getAllCrimenes")]
#[candid::candid_method(query, rename = "getAllCrimenes")]
async fn _cdk_user_defined_getAllCrimenes() -> (Vec<String>) {
    let interpreter = unsafe { INTERPRETER_OPTION.as_mut() }
        .unwrap_or_trap("SystemError: missing python interpreter");
    let vm = &interpreter.vm;
    let params = ();
    call_global_python_function("getAllCrimenes", params)
        .await
        .unwrap_or_trap()
}
#[ic_cdk_macros::query(name = "getCrimenesByZona")]
#[candid::candid_method(query, rename = "getCrimenesByZona")]
async fn _cdk_user_defined_getCrimenesByZona(_cdk_user_defined_zona: String) -> (Vec<String>) {
    let interpreter = unsafe { INTERPRETER_OPTION.as_mut() }
        .unwrap_or_trap("SystemError: missing python interpreter");
    let vm = &interpreter.vm;
    let params = (_cdk_user_defined_zona
        .try_into_vm_value(vm)
        .unwrap_or_trap(),);
    call_global_python_function("getCrimenesByZona", params)
        .await
        .unwrap_or_trap()
}
#[ic_cdk_macros::query(name = "getZonasByCrimen")]
#[candid::candid_method(query, rename = "getZonasByCrimen")]
async fn _cdk_user_defined_getZonasByCrimen(_cdk_user_defined_crimen: String) -> (Vec<String>) {
    let interpreter = unsafe { INTERPRETER_OPTION.as_mut() }
        .unwrap_or_trap("SystemError: missing python interpreter");
    let vm = &interpreter.vm;
    let params = (_cdk_user_defined_crimen
        .try_into_vm_value(vm)
        .unwrap_or_trap(),);
    call_global_python_function("getZonasByCrimen", params)
        .await
        .unwrap_or_trap()
}
#[ic_cdk_macros::update(name = "set")]
#[candid::candid_method(update, rename = "set")]
async fn _cdk_user_defined_set(
    _cdk_user_defined_zona: String,
    _cdk_user_defined_crimen: String,
) -> () {
    let interpreter = unsafe { INTERPRETER_OPTION.as_mut() }
        .unwrap_or_trap("SystemError: missing python interpreter");
    let vm = &interpreter.vm;
    let params = (
        _cdk_user_defined_zona
            .try_into_vm_value(vm)
            .unwrap_or_trap(),
        _cdk_user_defined_crimen
            .try_into_vm_value(vm)
            .unwrap_or_trap(),
    );
    call_global_python_function("set", params)
        .await
        .unwrap_or_trap()
}
#[derive(
